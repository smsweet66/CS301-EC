...
@app.route('/HW1')
def homework1():
	return '<pre>' + str(db.CS301.count_documents({}))
...
@app.route('/HW7')
def homework7():
	return '<pre>' + str(db.CS301.count_documents({'funding_rounds.raised_amount': {'$gt': 5000000}}))
...
@app.route('/HW12')
def homework12():
	return '<pre>' + str(db.CS301.count_documents({'screenshots.attribution': None}))
...
def listCompaniesByYear(yearFounded):
	if len(yearFounded) != 4:
		return '<pre>Year must be a four digit number'
...
def countCompaniesByYear(yearFounded):
	if len(yearFounded) != 4:
		...
		return f'<pre>There were {count} companies founded in {yearFounded}'
...
def crunchbaseRedirect(companyName):
	...
	if company:
		...
		return f'<pre>There is no company by the name {companyName}'