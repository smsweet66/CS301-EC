...
def homework6():
	output = ''
	for document in db.CS301.find({'founded_month': {'$exists':'false'}}, {'_id':1}):
		output += str(document) + '<br>'
...
def homework9():
	output = ''
	for document in db.CS301.find({'$or': [{'founded_year': {'$gt':2012}}, {'founded_year': {'$lt':1805}}]},
...
def homework10():
	output = ''
	for document in db.CS301.find({'founded_year': 1800, 'products.name': {'$exists': 'true'}},
